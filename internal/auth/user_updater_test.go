...
func TestUserUpdater(t *testing.T) {
	...
	t.Run("should_return_error_if_user_not_exists_in_repsository", func(t *testing.T) {
		i := auth.UserUpdaterInput{
			UserID:      "1D",
			Username:    "António",
			Email:       "gomes@gmail.com",
	...
	t.Run("should_return_error_if_is_not_manager_to_update_user", func(t *testing.T) {
		i := auth.UserUpdaterInput{
			UserID:      "2",
			ManagerID:   "3",
			Username:    "Ana",
			Email:       "james2@gmail.com",
			PhoneNumber: "943123467",
			Role:        auth.RoleReceptionist.String(),
		}
